...
pfah.remember = function () {
  $('.pfah-input').each(function () {
    if ($(this).closest('.pfahWrapper').data('remember') !== 'no') {
...
// initialize
pfah.init = function () {
  if ($('.pfahWrapper').length > 0) {
    var n = $('.pfahWrapper:last').data('style') === 'no'
    // default style
    if (!n) pfah.asset('style', 'pfah')
    // all forms
    $('.pfahWrapper').each(function () {
      // customize theme
      if (!n) {
  ...
  // debounce
  if ($('.pfahWrapper[data-remember="no"]').length < $('.pfahWrapper').length) pfah.asset('vendor', 'jquery.ba-throttle-debounce.min')
  // set value
  pfah.remember()
}
...
$(function () {
  ...
  // submit form
  $('body').on('submit', '.pfahWrapper', function (e) {
    e.preventDefault()
    if ($(this).data('recaptcha')) {
  ...
  $('body').on('click', '.pfah-error', function () {
    var f = $(this).closest('.pfahWrapper')
